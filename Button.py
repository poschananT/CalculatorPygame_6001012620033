    ...
    def creat_button(self):
        ...
        for i in Button.button:
            if pygame.mouse.get_pressed()[0] == 1:  # when you click
                if Button.button[i][0] + 100 > pos[0] > Button.button[i][0] and Button.button[i][1] + 100 > pos[1] > Button.button[i][1]:     # input x, y values for draw a rectangle and change the color
        ...
        for i in Button.button:
            if pygame.mouse.get_pressed()[0] == 1:
                if Button.button[i][0] + 100 > pos[0] > Button.button[i][0] and Button.button[i][1] + 100 > pos[1] > Button.button[i][1]:
                    return i
        # the value of each button to use
